...
#<malloc.h> include <ctype.h> //isalnum
...
int sf_print(); function_t scriptfunctions[] = {
	{"print", sf_print},
	{NULL, 0}
};
typedef struct {
...
ipr_t ipr; int ipr_getnext(ipr_t* ipr) {
	if(ipr == NULL)
		return -1;
...
var_t *ipr_convert_to_var(ipr_t *ipr, char *str) {
	...
	if(isinteger(str)) {
		...
		for(int i = 0; i < ipr->numvars; i++) {
			...
			//printf("ipr->variables[i]->name = %s CMP str = 
...
void interpret(ipr_t *ipr) {
	...
	for(int i = 0; i < sizeof(ipr->args) / sizeof(ipr->args[i]); 
i++)
		ipr->args[i]=NULL;
	ipr->argcount=0;
	
	for(int i = 0; i < sizeof(ipr->strings) / 
sizeof(ipr->strings[i]); i++)
		ipr->strings[i] = NULL;
	ipr->numstrings = 0;
	
	for(int i = 0; i < sizeof(ipr->variables) / 
sizeof(ipr->variables[i]); i++)
...
ipr_getstring(ipr);
				...
				for(int i = 0; i < ipr->numvars; i++) {
					...
					if(!strcmp(ipr->variables[i]->name, 
varname)) {
						printf("variable '%s' 
already exists!", varname);
						v_exists=1;
						break;
					}
...
ipr->variables[i]->name, varname);
				...
				var_t *var = 
(var_t*)malloc(sizeof(var_t));
				memset(var,0,sizeof(var_t));
				var->type=VT_NULL;
				strncpy(var->name, varname, 
sizeof(var->name) - 1);
...
varname, next);
				...
				if(ch == '=') {
					int v_found=0;
					var_t *v = NULL;
					for(int i = 0; i < ipr->numvars; 
i++) {
						v = ipr->variables[i];
						
						if(!strcmp(ipr->string, 
ipr->variables[i]->name)) {
							
							if(ch == '=') {
								int next 
= ipr_getnextnonspace(ipr);
...
== '"') {
									...
									const 
char *val = ipr_getstringnspaces(ipr);
...
!= '"') {
									...
									scriptstring_t 
*ss = (scriptstring_t*)malloc(sizeof(scriptstring_t));
									strncpy(ss->string, 
val, sizeof(ss->string) - 1);
									ss->string[sizeof(ss->string) 
- 1] = '\0';
									ss->index 
= ipr->numstrings;
									
									ipr->strings[ipr->numstrings++] 
= ss;
...
"%s", ipr->string);
					...
					const char *args = 
ipr_getstringnspaces(ipr);
					
					char tempstring[128] = {0};
					int tempsize = 0;
					int terr=0;
					for(int i = 0; i < strlen(args); 
i++) {
						if(is_space(args[i]))
							continue;
						...
						if(args[i] == ',') {
							...
							var_t *cv = 
ipr_convert_to_var(ipr, tempstring);
							
							if(cv == NULL) {
								terr=1;
								printf("could 
not convert string to argument!\n");
								break;
							}
							
							ipr->args[ipr->argcount++] 
= cv;
...
ipr_convert_to_var(ipr, tempstring);
					...
					for(int i = 0; 
scriptfunctions[i].name; i++) {
						
						if(!strcmp(scriptfunctions[i].name, 
fname)) {
...
call!\n");
					...
					var_t *v = ipr_getvar(ipr, 
ipr->string);
					
					if(v == NULL) {
						printf("variable %s does 
not exist\n", ipr->string);
						break;
					}
					
					int next = 
ipr->scriptbuf[ipr->curpos];
